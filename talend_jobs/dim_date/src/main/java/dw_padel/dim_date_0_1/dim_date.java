	...
	public class ContextProperties extends PropertiesWithType {
		...
		public void synchronizeContext(){
			...
			if(data_start != null){
					...
					String pattern_data_start = "yyyy-MM-dd HH:mm:ss";
					String value_data_start = "1980-01-01 00:00:00";
					String[] parts_data_start = value_data_start.split(";");
					if(parts_data_start.length > 1){
						pattern_data_start = parts_data_start[0];
						this.setProperty("data_start", pattern_data_start + ";" + FormatterUtils.format_Date(data_start, pattern_data_start));
					}else{
						this.setProperty("data_start", FormatterUtils.format_Date(data_start, pattern_data_start));
					}
...
public static class dimStruct implements routines.system.IPersistableRow<dimStruct> {
    final static byte[] commonByteArrayLock_DW_PADEL_dim_date = new byte[0];
    static byte[] commonByteArray_DW_PADEL_dim_date = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_DW_PADEL_dim_date.length) {
				if(length < 1024 && commonByteArray_DW_PADEL_dim_date.length == 0) {
   					commonByteArray_DW_PADEL_dim_date = new byte[1024];
				} else {
   					commonByteArray_DW_PADEL_dim_date = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_DW_PADEL_dim_date, 0, length);
			strReturn = new String(commonByteArray_DW_PADEL_dim_date, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_DW_PADEL_dim_date, 0, length);
			strReturn = new String(commonByteArray_DW_PADEL_dim_date, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_DW_PADEL_dim_date) {
...
String dbUser_tDBOutput_1 = null;
	...
	final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:YGJWxnmFBbAVREgyQpL8KV/3q3SBMMCHZZz047mmqJvV04A=");
...
if(dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
	...
	int count_tDBOutput_1=0;
            ...
            try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
                stmtCreate_tDBOutput_1.execute("CREATE TABLE [" + tableName_tDBOutput_1 + "]([date_key] INT ,[full_date] DATE ,[annee] INT ,[mois] INT ,[nom_mois] VARCHAR(20)  ,[jour_semaine] VARCHAR(20)  ,[trimestre] INT ,primary key([date_key]))");
            }
        java.sql.PreparedStatement pstmt_tDBOutput_1 = conn_tDBOutput_1.prepareStatement("SELECT COUNT(1) FROM [" + tableName_tDBOutput_1 + "] WHERE [date_key] = ?");
        resourceMap.put("pstmt_tDBOutput_1", pstmt_tDBOutput_1);
        String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1 + "] ([date_key],[full_date],[annee],[mois],[nom_mois],[jour_semaine],[trimestre]) VALUES (?,?,?,?,?,?,?)";
        java.sql.PreparedStatement pstmtInsert_tDBOutput_1 = conn_tDBOutput_1.prepareStatement(insert_tDBOutput_1);
        resourceMap.put("pstmtInsert_tDBOutput_1", pstmtInsert_tDBOutput_1);
        String update_tDBOutput_1 = "UPDATE [" + tableName_tDBOutput_1 + "] SET [full_date] = ?,[annee] = ?,[mois] = ?,[nom_mois] = ?,[jour_semaine] = ?,[trimestre] = ? WHERE [date_key] = ?";
...
int nb_max_row_tRowGenerator_2 = 35000;


class tRowGenerator_2Randomizer {
	public java.util.Date getRandomDATE() {
		
		return TalendDate.addDate(context.data_start ,Numeric.sequence("s2",1,1)-1 ,"dd" );
...
// # Output table : 'dim'
dim_tmp.date_key = Numeric.sequence("s1",1,1) ;
dim_tmp.full_date = row1.DATE ;
dim_tmp.annee = TalendDate.getPartOfDate("YEAR", row1.DATE) ;
dim_tmp.mois = TalendDate.getPartOfDate("MM", row1.DATE) ;
dim_tmp.nom_mois = TalendDate.formatDate("MMMM", row1.DATE) ;
dim_tmp.jour_semaine = TalendDate.formatDate("DD", row1.DATE) ;
dim_tmp.trimestre = (Integer.parseInt(TalendDate.formatDate("MM", row1.DATE)) - 1) / 3 + 1;
dim = dim_tmp;
// ###############################
...
if(dim != null) { 
	...
	currentComponent="tDBOutput_1";
        ...
        whetherReject_tDBOutput_1 = false;
                    ...
                    if(dim.date_key == null) {
pstmt_tDBOutput_1.setNull(1, java.sql.Types.INTEGER);
} else {pstmt_tDBOutput_1.setInt(1, dim.date_key);
}
            ...
            if(checkCount_tDBOutput_1 > 0) {
                        if(dim.full_date != null) {
pstmtUpdate_tDBOutput_1.setTimestamp(1, new java.sql.Timestamp(dim.full_date.getTime()));
} else {
...
} else {pstmtUpdate_tDBOutput_1.setInt(2, dim.annee);
}

                        if(dim.mois == null) {
pstmtUpdate_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setInt(3, dim.mois);
}

                        if(dim.nom_mois == null) {
pstmtUpdate_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(4, dim.nom_mois);
}

                        if(dim.jour_semaine == null) {
pstmtUpdate_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtUpdate_tDBOutput_1.setString(5, dim.jour_semaine);
}

                        if(dim.trimestre == null) {
pstmtUpdate_tDBOutput_1.setNull(6, java.sql.Types.INTEGER);
} else {pstmtUpdate_tDBOutput_1.setInt(6, dim.trimestre);
}
...
} else {pstmtUpdate_tDBOutput_1.setInt(7 + count_tDBOutput_1, dim.date_key);
}
...
} else {pstmtInsert_tDBOutput_1.setInt(1, dim.date_key);
}

                        if(dim.full_date != null) {
pstmtInsert_tDBOutput_1.setTimestamp(2, new java.sql.Timestamp(dim.full_date.getTime()));
} else {
...
} else {pstmtInsert_tDBOutput_1.setInt(3, dim.annee);
}

                        if(dim.mois == null) {
pstmtInsert_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
} else {pstmtInsert_tDBOutput_1.setInt(4, dim.mois);
}

                        if(dim.nom_mois == null) {
pstmtInsert_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(5, dim.nom_mois);
}

                        if(dim.jour_semaine == null) {
pstmtInsert_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
} else {pstmtInsert_tDBOutput_1.setString(6, dim.jour_semaine);
}

                        if(dim.trimestre == null) {
pstmtInsert_tDBOutput_1.setNull(7, java.sql.Types.INTEGER);
} else {pstmtInsert_tDBOutput_1.setInt(7, dim.trimestre);
}
 ...
 */
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = dim_date.class.getClassLoader().getResourceAsStream("dw_padel/dim_date_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
            ...
            class ContextProcessing {
                private void processContext_0() {
                        context.setContextType("data_start", "id_Date");
                        if(context.getStringValue("data_start") == null) {
                            context.data_start = null;
                        } else {
                            String context_data_start_value = context.getProperty("data_start");
                            try{
                                if (context_data_start_value == null){
                                    context_data_start_value = "";
                                }
                                int context_data_start_pos = context_data_start_value.indexOf(";");
                                String context_data_start_pattern =  "yyyy-MM-dd HH:mm:ss";
                                if(context_data_start_pos > -1){
                                    context_data_start_pattern = context_data_start_value.substring(0, context_data_start_pos);
                                    context_data_start_value = context_data_start_value.substring(context_data_start_pos + 1);
                                }

                                context.data_start=(java.util.Date)(new java.text.SimpleDateFormat(context_data_start_pattern).parse(context_data_start_value));

                            } catch(ParseException e) {
                                try { 
                                    long context_data_start_longValue = Long.valueOf(context_data_start_value);
                                    context.data_start = new java.util.Date(context_data_start_longValue);
                                } catch (NumberFormatException cantParseToLongException) {
                                        System.err.println(String.format("Null value will be used for context parameter %s: %s", "data_start", "Can't parse date string: " + e.getMessage() + " and long: " + cantParseToLongException.getMessage()));
                                    context.data_start=null;
                                }
                            }
                        }
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("data_start")) {
                context.data_start = (java.util.Date) parentContextMap.get("data_start");
            }
        }
...
/************************************************************************************************
 *     65124 characters generated by Talend Open Studio for Data Integration 
 *     on the 29 avril 2026 à 03:23:32 WAT
 ************************************************************************************************/